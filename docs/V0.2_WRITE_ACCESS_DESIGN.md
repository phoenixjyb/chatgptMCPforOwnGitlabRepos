# v0.2.0 Design: ChatGPT Read MCP + Local Codex Coding Engine

## Product goal

v0.2 keeps the normal ChatGPT integration **read-only** on personal Pro, and adds a separate local coding engine that Codex or a human can invoke from the terminal.

This gives the maximum practical capability without requiring a Business workspace or OpenAI model API billing.
...
gitlab-agent
    ...
    ├── commit
    ├── push feature branch
    └── create GitLab MR on first push
```
...
### 2. Local package: `gitlab_agent`

The new package under `src/gitlab_agent/` contains:
...
- `cli.py` — JSON-oriented command line interface for Codex and humans.
...
A useful prompt to Codex is:

> Use the `gitlab-agent` CLI. Create an isolated worktree for `team/project-a` from `main`, investigate the timeout bug, make the minimal fix, run the relevant tests, show me the diff, and only after tests pass commit and create an MR.

Codex remains free to inspect/edit the worktree directly if appropriate, but the CLI gives it a consistent lifecycle, project allowlist, branch policy, and safe Git push path.
