# v0.2.0 Design: ChatGPT Read MCP + CodingAgent

## Product goal

v0.2 keeps the normal ChatGPT integration **read-only** on personal Pro, and adds **CodingAgent**, an agent-neutral local coding layer that can hand managed worktrees to Codex CLI, GitHub Copilot CLI, or future coding agents.
...
### 2. CodingAgent + local package `gitlab_agent`
...
- `codingagent_cli.py` — agent-neutral `codingagent` entry point.
...
`codingagent` is the user-facing orchestration layer. `gitlab-agent` remains the lower-level GitLab/worktree lifecycle controller.
...
## CodingAgent backend interaction

Create a Codex handoff:

```bash
codingagent task team/project-a --agent codex --task fix-timeout --goal "Fix the timeout bug"
```

Or hand the same workflow to GitHub Copilot CLI:

```bash
codingagent task team/project-a --agent copilot --task fix-timeout --goal "Fix the timeout bug"
```

Coding backends may inspect/edit the returned worktree directly. CodingAgent and `gitlab-agent` provide a consistent lifecycle, project allowlist, branch policy, test/diff review path, safe push/MR creation, MR iteration, and MR/branch recovery.
